...
<div class="container my-4" style="max-width: 950px;">
    ...
    <div id="invoiceContent" class="rounded shadow p-5 border-top bg-white">
        <style>
            ...
            #invoiceContent textarea { resize: none; }
            ...
            <div class="col-6">
                ...
                <label for="logoInput" class="mb-3">
                    <div class="rounded p-2 text-center "
                        ...
                        <img id="logoPreview" src="{{ asset('storage/' . $invoice->logo_path) }}" 
        ...
        <!-- Totals -->
        @php
            $taxAmount = $invoice->tax_type === 'percentage'
                ? ($subtotal * $invoice->tax_value) / 100
                : $invoice->tax_value;
        ...
        <div class="row mt-4">
            ...
            <div class="col-5">
                <table class="table table-sm">
                    ...
                    <tr>
                        <td><strong>Tax 
                            {{ $invoice->tax_type === 'percentage' ? "({$invoice->tax_value}%)" : '' }}:</strong></td>
                        <td class="text-end">{{ $invoice->currency ?? '$' }} {{ number_format($taxAmount, 2) }}</td>
                    </tr>
                    <tr class="table-dark">
                        <td><strong>Total:</strong></td>
                        <td class="text-end"><strong>{{ $invoice->currency ?? '$' }} {{ number_format($total, 2) }}</strong></td>
                    </tr>
                </table>
            </div>
        </div>
